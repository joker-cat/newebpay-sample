...
app.post("/upload", upload.single("video"), (req, res) => {
  ...
  const outputDir = `hls/${path.parse(req.file.filename).name}`;

  convertToHLS(videoPath, outputDir, (err) => {
      if (err) {
          console.error(err);
          return res.json({ success: false, message: "轉換失敗" });
      }

      res.json({
          success: true,
          url: `http://localhost:5000/${outputDir}/index.m3u8`
      });
  });
});
